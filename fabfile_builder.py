...
"""
from fabric.api import local, env, local as run, put, lcd as cd, task, sudo, settings, warn_only, lcd, path, get, execute
...
def package_all(s3url):
    ...
    with cd(CONDA_BUILD_DIR):
        ...
        local("aws s3 cp /tmp/conda/condaruntime.tar.gz {}".format(s3url))
...
def get_runtime_pip_freeze(conda_install_dir):
    return run("{}/bin/pip freeze 2>/dev/null".format(conda_install_dir), capture=True)

@task
def get_preinstalls(conda_install_dir):
    return run('{}/bin/python -c "import pkgutil;import json;print(json.dumps([(mod, is_pkg) for _, mod, is_pkg in pkgutil.iter_modules()]))"'.format(conda_install_dir), capture=True)

@task
def get_conda_root_env(conda_install_dir):
    return run("{}/bin/conda env export -n root 2>/dev/null".format(conda_install_dir), capture=True)
