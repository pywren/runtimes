...
PIP_OPT_SET = ['cvxpy' ]
CONDA_BIG = ['scipy', 'cvxopt', 'scikit-image', 'scikit-learn']
...
RUNTIMES = {'minimal' : {'pythonvers' : ["3.7"],
            ...
            'default' : {'pythonvers' : ["3.7"],
                         'packages' : {
                             ...
                             'pip_install' : PIP_DEFAULT_LIST,
                             'pip_upgrade' : PIP_DEFAULT_UPGRADE_LIST}},
            'too_big_do_not_use' : {'pythonvers' : ["3.7"],
                         'packages' : {
                             'conda_install' : CONDA_DEFAULT_LIST + CONDA_BIG,
                             'pip_install' : PIP_DEFAULT_LIST,
                             'pip_upgrade' : PIP_DEFAULT_UPGRADE_LIST}}
             }
...
S3_BUCKET = "s3://pywren-runtimes-public-us-west-2"
S3URL_STAGING_BASE = S3_BUCKET + "/pywren.runtime.staging"
S3URL_BASE = S3_BUCKET + "/pywren.runtimes"

def get_staged_runtime_url(runtime_name, runtime_python_version):
    s3url = "{}/{}_{}".format(S3URL_STAGING_BASE, runtime_name, runtime_python_version)
